...
- научиться пользоваться gdb (`gdb-multiarch` в контейнере)
    - https://pdos.csail.mit.edu/6.828/2023/labs/gdb.html
...
Причина указана: 0xd (запись/чтение по недопустимому адресу?)
...
А уже после — лежит в каневе. Дело в том, что в какой-то момент адрес
`Sz_info.alloc` становится `0x1`. А именно он так устанавливается в CAS в
`acuire(&p->lock)` (пригодился `(gdb) watch bd_sizes[7].alloc`). 

Оказывается, что в какой-то момент один тред, итерируется по списку процессов,
а другой удаляет процесс выполняющий `ls` из списка и деаллоцирует его
структуру, добавляя блок в список свободных `bd_sizes[7].free`.
Другой же процесс имеет ссылку на удаленный старый процесс, но так вышло, что в
нем `Proc_list` и `Bd_list` занимают одинаковы офсеты, а уже `p->lock`
находится на офсете `Sz_info.alloc`, поэтому захват блокировки рушит структуру
аллокатора.

Для выявления этой проблемы можно в gdb поставить watchpoint на чтение и/или
запись в этот самый указатель `alloc` или условный брекпоинт, когда значение
становится `0x1`.

Вывод: сделать хорошую синхронизацию потоков, ввести счетсчик ссылок элементов
списка, lockfree список..., rwlock...

- - -

После долгих попыток написать хорошую синхронизацию, rwlock, динамический
массив, были выявлены некоторые проблемы в их реализации с помощью локов в xv6.

Синхронизированный список таки написан, по факту это глобальная блокировка с
честным подсчетом ссылок на элементы списка. По другому, кажется делать смысла
не имеет, т.к. блокировки в xv6 очень топорные (отключение прерываний).
Тем не менее такое решение работает и проходит все `usertests` тесты, кроме `forktest`,
потому что он проверяет, что количество новых процессов ограничено. 
Так же оказывается, что `forkforkfork` отрабатывает за ~5-10 минут, в
зависимости от того, как вы вкачали удачу.
