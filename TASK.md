...
[//]: Доделать, с этого места. счётчик ссылок в kalloc.c: kmem.refs
...
Пока мы живем в ванильной реализации xv6, а не в доработанной версии с
неограниченным числом процессов, можно сказать, что мы имеем не более чем
#NPROC=64 ссылок на одну страницу, и поэтому достаточно 1 байта на каждую
страницу для хранения счетчика.

Задание почти доделано, остается продумать API для изменения счетчика ссылок:
vm.c не должен включать kalloc.с, в котором объявлен kmem, т.е. нужно сделать 
отдельные функции в kalloc.c (добавить в defs.h) для подобных операций.
Еще нужно соблюсти логику этих изменений счетчика ссылок в vm.c
...
Для реализации ленивой аллокации нужно будет добавить в PTE страниц, в раздел RSV свой 
бит отвечающий за возможность аллоцировать позже.
